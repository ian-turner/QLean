...
@[simp] theorem eval_id  : eval (.id : Circuit n) = 1 := rfl
@[simp] theorem eval_gate (U : QMatrix n) : eval (.gate U) = U := rfl
@[simp] theorem eval_seq (c₁ c₂ : Circuit n) : eval (.seq c₁ c₂) = eval c₂ * eval c₁ := rfl
@[simp] theorem eval_par {j k : ℕ} (c₁ : Circuit j) (c₂ : Circuit k) :
    eval (.par c₁ c₂) = kronQMatrix (eval c₁) (eval c₂) := rfl
...
@[simp] theorem wf_id   : Circuit.WF (.id : Circuit n) := trivial
@[simp] theorem wf_gate {U : QMatrix n} : Circuit.WF (.gate U) ↔ IsUnitary U := Iff.rfl
@[simp] theorem wf_seq  {c₁ c₂ : Circuit n} :
    Circuit.WF (.seq c₁ c₂) ↔ Circuit.WF c₁ ∧ Circuit.WF c₂ := Iff.rfl
@[simp] theorem wf_par  {c₁ : Circuit j} {c₂ : Circuit k} :
    Circuit.WF (.par c₁ c₂) ↔ Circuit.WF c₁ ∧ Circuit.WF c₂ := Iff.rfl
...
theorem Circuit.eval_unitary (c : Circuit n) (h : c.WF) : IsUnitary (eval c) := by
  induction c with
  | id => unfold IsUnitary; simp [Matrix.conjTranspose_one]
  | gate U => exact h
  | seq c₁ c₂ ih₁ ih₂ =>
    obtain ⟨h₁, h₂⟩ := h
    simp only [eval_seq]
