...
def Circuit.maps (C : Circuit n) (φ ψ : QState n) : Prop :=
  eval C * φ = ψ

@[simp] theorem Circuit.maps_iff (C : Circuit n) (φ ψ : QState n) :
    C.maps φ ψ ↔ eval C * φ = ψ := Iff.rfl

/-- `C.prepares ψ` holds when `C` produces state `ψ` from the all-zeros input `|0…0⟩`. -/
abbrev Circuit.prepares (C : Circuit n) (ψ : QState n) : Prop :=
  C.maps (ket 0) ψ

theorem Circuit.maps_id (φ : QState n) : (1 : Circuit n).maps φ φ := by
  simp [Circuit.maps]

/-- If `C₁` maps `φ` to `χ` and `C₂` maps `χ` to `ψ`, then `C₁ * C₂` maps `φ` to `ψ`. -/
theorem Circuit.maps_comp {C₁ C₂ : Circuit n} {φ χ ψ : QState n}
