...
/-- The identity circuit on one qubit; a space-saving alias for `(1 : QCircuit 1)`. -/
abbrev id1 : QCircuit 1 := 1
...
theorem rz_commutes_cnot (θ : ℝ) :
  ...
  have rz_phase : ∀ x : Fin (2 ^ 1),
    ...
    grw [QCircuit.par_action_tensor, hφ, QCircuit.id_action, QState.smul_tensor_left]
  ...
  have order₁ : ((RzGate θ ⊗ id1) * CNOTGate) * (❘a⟩ ⊗ ❘b⟩) ≈ φ • (❘a⟩ ⊗ ❘a + b⟩) := by
    grw [QCircuit.seq_action, CNOTGate_basis_tensor, rz_phase (a + b)]
  ...
  have order₂ : (CNOTGate * (RzGate θ ⊗ id1)) * (❘a⟩ ⊗ ❘b⟩) ≈ φ • (❘a⟩ ⊗ ❘a + b⟩) := by
    grw [QCircuit.seq_action, rz_phase b, QCircuit.apply_smul, CNOTGate_basis_tensor]
